...
void GLFunction::setupPolygons() {
    ...
    this->polyBuffer->resize(GL_DYNAMIC_DRAW,psize*(2*sizeof(float)+sizeof(int)));
    this->polyBuffer->setData(GL_DYNAMIC_DRAW,data->verts.data(),psize*sizeof(float)*2,0);// vertex of polygons
    this->polyBuffer->setData(GL_DYNAMIC_DRAW,data->ids.data(),psize*sizeof(int),psize*sizeof(float)*2);// id of polygons
}

void GLFunction::setupPoints() {
    DataHandler * data = this->handler->dataHandler;
    pointsize = data->point.size() / 2;
    this->pbuffer->resize(GL_DYNAMIC_DRAW,pointsize*sizeof(float)*2);
    this->pbuffer->setData1(GL_DYNAMIC_DRAW, data->point.data(),pointsize*sizeof(float)*2,0,false);
